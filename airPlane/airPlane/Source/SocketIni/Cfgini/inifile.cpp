	...
	while( !end_of_string(p[i]) ) {
...

	   {
		...
		if('=' == p[j]) {
		 ...
		 if(keylen == j-newline_start && strncmp(key,p+newline_start,keylen)==0)

		 {
	//	 char testtest[100];
	//	 memcpy(testtest,p+newline_start,j-newline_start);
	//	 testtest[j-newline_start] = 0;
	//	printf("\n***key=%s,kenlen=%d,inicmp=%s,inilen=%d**\n",key,keylen,testtest,j-newline_start);
		  //find the key ok
